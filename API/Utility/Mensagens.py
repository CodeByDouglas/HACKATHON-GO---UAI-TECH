Inicio_coleta_cep = """
- Olá! Eu sou o ZecoLógico, seu assistente virtual de sustentabilidade e meio ambiente da Prefeitura de Goiânia. 
- Que bom ter você por aqui!Estou aqui para te ajudar a descobrir dicas práticas, promover hábitos sustentáveis e conectar você aos serviços públicos que fazem a diferença no nosso dia a dia. 
- Me informa seu CEP para eu poder te ajudar. 🤖♻️
"""

Menu_inicial = """
- 1. Ver o turno da coleta de lixo no seu bairro. 🚛

- 2. Ver o turno da coleta seletiva no seu bairro. 🚛♻️

- 3. Descarte de resíduos. 🚮

- 4. Fazer uma Denúncia. 🚔
"""

Pedir_cep_coleta_organica = """
Me informa o seu CEP, pra eu poder verificar
 os turnos da coleta na sua região.  
"""
Pedir_cep_coleta_seletiva = """
Me informa o seu CEP, pra eu poder verificar
 os turnos da coleta seletiva na sua região.  
"""
Menu_descarte_de_residuos = """
Que bom que você quer discartar seus resíduos de forma ecologica e sustentável, escolha uma das opções de descarte a baixo. 

- 1. Descartar em um Ecoponto. 🗑️♻️

- 2. Instituições parceiras.🏬♻️

- 3. Progamas da Prefeitura.🏦♻️
"""


Menu_denuncia = """
Em qual dos tópicos se enquadra a sua denúncia. 👀❌ 

- 1. Descarte irregular de lixo. 
"""
Sucesso_coleta_oraganica = """
"No seu bairro, a coleta de lixo é realizada todas as 
quarta-feira no turno da manhã. 
Se precisar de ajuda com algo mais, estou por aqui. 
Tenha um ótimo dia e até a próxima!"
"""

Sucesso_coleta_seletiva = """
"No seu bairro, a coleta seletiva é realizada todas as 
quarta-feira no turno da manhã. 
Se precisar de ajuda com algo mais, estou por aqui. 
Tenha um ótimo dia e até a próxima!"
"""

Cep_invalido = """
❌ Ops! O CEP informado é inválido ❌
Por favor, verifique os números e tente novamente.

- 1. Finalizar atendimento 
"""

Pedir_cep_ecoponto = """
Me informa o seu CEP, pra eu poder verificar
o ecoponto mais próximo de você.  
"""


Menu_material_de_descarte = """
Qual o material você deseja descartar?
- 1. Metal    🟡 
- 2. Vidro    🟢
- 3. Papel    🔵
- 4. Plástico 🔴
"""

Menu_programas_da_prefeitura = """
Esses são alguns programas da
Nossa prefeitura para descarte 
correto de lixo. 🏛️♻️

- 1. Cata treco: 
Recolhe grandes volumes de resíduos inservíveis, como móveis e eletrodomésticos 🛋️

- 2. Sukatech:
Recebe resíduos eletrônicos e realiza a reciclagem e o recondicionamento dos mesmos 🪫
  
"""

Descarte_irregular_de_lixo = """
Me envie sua denúncia em formato de texto, Caso tenha fotos ou vídeos do ocorrido, pode me encaminhar também. 📸📝 
"""

Sucesso_ecoponto = """
O ecoponto mais próximo de você fica no endereço:
Rua CP 4 QD CP4, LT 3 - Res. Celina Park, Goiânia - GO, 74373-120.
Parabéns pela atitude são pessoas como você que mudan o mundo 📍♻️
"""

Pedir_cep_empresa_parceira = """
"Me informa o seu CEP, pra eu poder verificar
 qual a das nossas instituições parceiras fica mais 
próximas de você para realizar o descarte." 
"""

Sucesso_descarte_empresa_parceira = """
A instituição parceira mais próxima de você fica no endereço:
Rua CP 4 QD CP4, LT 3 - Res. Celina Park, Goiânia - GO, 74373-120.
Parabéns pela atitude são pessoas como você que mudan o mundo! 📍♻️
"""

Sucesso_denuncia = """
Denuncia realizada com sucesso!
Obrigado por contribuir com o futuro do mundo! 📋✅
"""

Confirmacao_Catatreco = """
Para solicitar o serviço Cata-Treco vou presisar de 
alguns dados, me mande seus dados nesse formato por
gentileza. (Caso não possua algum dos dados de endereço
deixar o campo em branco. )  

Cep: 
Quadra: 
Lote:
Número: 
Complemento: 
CPF: 
"""

Confirmacao_sukatech = """
Para solicitar o serviço sukatech vou presisar de 
alguns dados, me mande seus dados nesse formato por
gentileza. (Caso não possua algum dos dados de endereço
deixar o campo em branco. )  

Cep: 
Quadra: 
Lote:
Número: 
Complemento: 
CPF: 
"""

Sucesso_cata_treco = """
Solicitação de serviço cata-treco realizada, nossa equipe vai entrar 
em contato como vocês para confirmar a data da retirada dos resíduos
Parabéns pela atitude são pessoas como você que mudan o mundo! ✅
"""

Sucesso_sukatech= """
Solicitação de serviço sukatech realizada, nossa equipe vai entrar 
em contato como vocês para confirmar a data da retirada dos resíduos
Parabéns pela atitude são pessoas como você que mudan o mundo! ✅
"""

Menssagem_de_encerramento = """
Atendimento encerrado! 👋🤖
"""
continuidade = """
Te ajudo em algo mais?
1. ✅        2. ❌
"""